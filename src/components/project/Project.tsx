...
const Project: FC<ProjectProps> = ({ project, index }) => {
    ...
    return (
        ...
        >
            ...
            >
                <div
                    className={`h-full flex flex-col  justify-center ${project.phone ? 'w-fit  items-end' : 'md:min-w-[0%] md:w-full items-center'
                        }${project.title.includes("Speed") ? 'md:w-full' : ''}`}
                >
                    <Slideshow images={project.images} phone={project.phone} alt={project.title} />
                </div>
                <div
                    className={`flex flex-col rounded-md justify-center bg-indigo p-10  bg-opacity-25 h-fit items-center ${project.phone ? 'md:w-full ' : 'md:min-w-2/6 md:w-full '
                        } ${!isEven && 'md:text-end'}`}
                >
